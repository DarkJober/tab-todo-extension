...
function tabsCreate(createProperties) {
  try {
    const maybePromise = ext.tabs.create(createProperties);
    if (isThenable(maybePromise)) {
      return maybePromise;
    }
  } catch {
...
function tabsRemove(tabId) {
  try {
    const maybePromise = ext.tabs.remove(tabId);
    if (isThenable(maybePromise)) {
      return maybePromise;
    }
  } catch {
  ...
  return new Promise((resolve, reject) => {
    ext.tabs.remove(tabId, () => {
      const err = ext.runtime?.lastError;
      if (err) {
        reject(new Error(err.message || "tabs.remove failed"));
        return;
      }
      resolve();
    });
